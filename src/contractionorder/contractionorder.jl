...
export parse_eincode, ContractionOrder, optimizecode_greedy
...
function optimizecode_greedy(code::EinCode{ixs, iy}, size_dict; method=MinSpaceOut()) where {ixs, iy}
    if length(ixs) < 2
        return code
    end
...
optimizecode_greedy(code::Int, size_dict; method=MinSpaceOut()) = code

function optimizecode_greedy(code::NestedEinsum, size_dict; method=MinSpaceOut())
    args = optimizecode_greedy.(code.args, Ref(size_dict); method=method)
    if length(code.args > 2)
        ...
        nested = optimizecode_greedy(code.eins, size_dict; method=method)
...
replace_args(nested::Int, trueargs) = args[nested]
